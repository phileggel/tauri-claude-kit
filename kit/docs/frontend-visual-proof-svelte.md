...
> Required by F18 in [`frontend-rules-svelte.md`](frontend-rules-svelte.md) § Tests.

Any change that touches a `.svelte`, `.css`, or visual asset file **MUST** include committed
...
<html lang="en">
  ...
  <body>
    <div id="root"></div>
    <script type="module" src="/src/__preview__/main.ts"></script>
  </body>
</html>
```

**`src/__preview__/main.ts`** — bootstraps the preview by mounting a `Preview.svelte` wrapper
that renders the component in every relevant state with hardcoded mock data. No Tauri `invoke()`
calls; the gateway pattern keeps components decoupled from the IPC layer.
...
**`src/__preview__/Preview.svelte`** — wraps each state in a `<div id="state-{name}">` so
Playwright can target it for a per-element screenshot:

```svelte
<script lang="ts">
  import MyComponent from "../features/domain/MyComponent.svelte";
</script>

<div id="state-idle" style="padding: 24px">
  <MyComponent
    isLoading={false}
    error={null}
...
<div id="state-loading" style="padding: 24px">
  <MyComponent isLoading={true} error={null} items={[]} />
</div>

<div id="state-error" style="padding: 24px">
...
<div
  role="dialog"
  aria-modal="true"
  class="relative bg-{surface-container-lowest} rounded-[28px] shadow-elevation-4 w-full max-w-2xl overflow-hidden flex flex-col"
>
...
Svelte mount + reactive updates, and `networkidle` is officially discouraged in Playwright.
